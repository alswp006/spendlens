 ...
 *   SpendLens -> the app's display name
 *   한국 직장인 10명 중 7명이 월말 '돈이 어디 갔지?'를 경험하는 가운데, 카드사 앱의 단순 내역 조회를 넘어 AI가 소비 패턴의 '낭비 구조'를 시각적으로 진단해주는 개인 재무 코치 앱    -> the one-line description
 */
...
export default function Home() {
  ...
  return (
    <ScreenScaffold
      top={<Top title={<Top.TitleParagraph>SpendLens</Top.TitleParagraph>} />}
    >
      {/* 시각 앵커: 헤드라인 + 카드 내 진입 버튼(부유 금지, display="block" 전체폭).
          데이터 앱이면 value를 <Amount typography="t1" />(핵심 숫자)로 교체하라. */}
      <SummaryHero
        label="SpendLens"
        value={<Paragraph.Text typography="t2">한국 직장인 10명 중 7명이 월말 '돈이 어디 갔지?'를 경험하는 가운데, 카드사 앱의 단순 내역 조회를 넘어 AI가 소비 패턴의 '낭비 구조'를 시각적으로 진단해주는 개인 재무 코치 앱</Paragraph.Text>}
